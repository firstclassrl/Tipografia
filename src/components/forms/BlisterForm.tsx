...
export const BlisterForm: React.FC<BlisterFormProps> = ({ orderNumber, onSave }) => {
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          <input
            ...
            onChange={handleInputChange}
            className="w-full px-4 py-3 backdrop-blur-sm bg-white/10 border border-white/30 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all text-white placeholder-white/50 font-medium"
            placeholder="Es. 100x60mm, 10 cavità"
          />
        </div>
      ...
      <div className="flex justify-end pt-8 border-t border-white/20">
        <button
          type="submit"
          disabled={loading}
          className="bg-gradient-to-r from-black/40 to-red-600/40 text-white px-10 py-4 rounded-xl hover:from-black/60 hover:to-red-600/60 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 flex items-center gap-3 font-bold text-lg shadow-lg hover:shadow-red-500/25 hover:-translate-y-1 border border-red-500/30"
        >
          {loading ? (
            <>
